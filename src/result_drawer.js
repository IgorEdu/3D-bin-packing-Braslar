...
function loadPacksInstanced(openPoints, packagesLoaded) {
    ...
    for (const pack in packagesLoaded) {
        const matrix = new THREE.Matrix4();

        // let packLoadedLength = loadedPacks[pack].length;
        let packLoadedLength = 1;

        let packSpecs = {
            parent_id: packagesLoaded[pack].parent_id,
            w: packagesLoaded[pack].w,
            h: packagesLoaded[pack].h,
            l: packagesLoaded[pack].l,
            color: packagesLoaded[pack].color
        }
        ...
        // for (let i = 0; i < packLoadedLength; i++) {
            let position = {
                x: packagesLoaded[pack].x,
                y: packagesLoaded[pack].y,
                z: packagesLoaded[pack].z
            }
